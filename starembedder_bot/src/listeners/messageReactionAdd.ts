...
import { Attachment, Events, MessageReaction, PartialMessageReaction, PartialUser, StickerFormatType, User } from 'discord.js';
	...
	public override async run(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) {
		...
		try {
			...
			const skullboardChannel =
				guild.channels.cache.get(config.skullboardChannelId) ?? (await guild.channels.fetch(config.skullboardChannelId).catch(() => null));
			...
			if (refId) {
				try {
					...
					replyPayload = {
						...
						attachment:
							refMsg.attachments.size > 0 || refMsg.embeds.some((e) => AUTO_EMBED_TYPES.has((e.data as { type?: string }).type ?? '')),
			...
			const payload: RenderPayload = {
				message: {
					...
					attachments: [...[...message.attachments.values()].map(attachmentToRender), ...autoEmbedAttachments],
